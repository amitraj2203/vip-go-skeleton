...
class MeprMembersTable extends WP_List_Table {
  ...
  public function get_sortable_columns() {
    return $sortable= array(
      ...
      'col_last_login_date' => array('last_login_date',true),
      'col_login_count' => array('login_count',true),
      'col_total_spent' => array('total_spent',true),
      'col_registered'  => array('registered',true),
    );
  }
  ...
  public function prepare_items() {
	...
	$search_field_val = !empty($_GET["search-field"])  ? esc_sql($_GET["search-field"])  : 'any';
   // $search_field = isset($this->db_search_cols[$search_field]) ? $this->db_search_cols[$search_field] : 'any';
	
	if($search_field_val == 'mepr_firm'){
			  $search_field = 'pm_mepr_firm.meta_value';
		  }else{
			  //$search_field = isset($tab->db_search_cols[$search_field_val]) ? $tab->db_search_cols[$search_field_val] : 'any';
			  $search_field = isset($this->db_search_cols[$search_field_val]) ? $this->db_search_cols[$search_field_val] : 'any';
		  }
