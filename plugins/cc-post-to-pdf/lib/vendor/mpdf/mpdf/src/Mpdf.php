...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function watermark($texte, $angle = 45, $fontsize = 96, $alpha = 0.2,$sectxt=0)
	{
		...
		if($sectxt==1){
			$this->Text($mid_x-$this->GetStringWidth($texte)/2+20,$wy-5,$texte,$OTLdata, $textvar);
		}else if($sectxt==2){
			$this->Text($mid_x-$this->GetStringWidth($texte)/2+20,$wy+8,$texte,$OTLdata, $textvar);
		}else{
			$this->Text($mid_x-$this->GetStringWidth($texte)/2+20,$wy-20,$texte,$OTLdata, $textvar);
		}
