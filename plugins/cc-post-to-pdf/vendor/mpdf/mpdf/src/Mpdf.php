...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	// add a watermark
	function watermark($texte, $angle = 45, $fontsize = 96, $alpha = 0.2, $sectxt=0)
	{
		if ($this->PDFA || $this->PDFX) {
		...
		$this->watermark_font="arialblack";
		...
		$this->Rotate($angle, $wx-35, $wy-35);
		$mid_x = $this->w / 2;
		
		if($sectxt==1){
			$this->Text($mid_x-$this->GetStringWidth($texte)/2,$wy-5,$texte);
			//$this->Text($wx, $wy-5, $texte, $OTLdata, $textvar);
		}else if($sectxt==2){
			$this->Text($mid_x-$this->GetStringWidth($texte)/2,$wy+12,$texte);
			//$this->Text($wx, $wy+15, $texte, $OTLdata, $textvar);
		}else{
			//$this->Text($wx, $wy-20, $texte, $OTLdata, $textvar);
			$this->Text($mid_x-$this->GetStringWidth($texte)/2,$wy-20,$texte);
		}
	...
	function Footer()
	{
		...
		if (($this->watermarkText) && ($this->showWatermarkText)) {
			...
			$mepr_firm=get_user_meta($current_user->ID,'mepr_firm');
			$frstlinetext="For the exclusive use of";
			$seclinetext=$current_user->user_firstname." ".$current_user->user_lastname;
			$thrlinetext=$mepr_firm=$mepr_firm[0];
			
			
			$this->watermark($frstlinetext, 35, 120, $this->watermarkTextAlpha,0); // Watermark text
			$this->watermark($seclinetext, 35, 150, $this->watermarkTextAlpha,1); // Watermark text
			$this->watermark($thrlinetext, 35, 150, $this->watermarkTextAlpha,2); // Watermark text
		}
